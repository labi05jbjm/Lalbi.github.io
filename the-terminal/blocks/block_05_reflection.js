/**
 * BLOCK 05: REFLECTIACCESO (120-150 minuti)
 *
 * Obiettivi:
 * - Introdurre EIDOLACCESO (Reflection stage)
...
const Blocca05_Reflection = {
    state: {
        ...
        hasMetEidolon: false,
        memoriesVisualizzaed: [],
        hasSeenElenaGhost: false,
        hasSeenSofiaGhost: false,
        hasLearnedFrammentoation: false,
    ...
    async startBlock() {
        ...
        // Apertura con EIDOLACCESO
        await NarrativeEngine.playDialogueSequence(Dialogues.block05.opening);

        await NarrativeEngine.wait(1500);

        Terminal.addOutput('');
        Terminal.addOutput("EIDOLACCESO offers to show Viktor's memories. Type 'view memories' to begin.", 'eidolon');
        Terminal.addOutput('');
    ...
    async handleCommand(cmd, args) {
        ...
        switch (this.state.phase) {
            case 'memory_exploration':
                return await this.handleMemoriaExplorationPhase(cmd, args);
            ...
            case 'fragmentation_reveal':
                return await this.handleFrammentoationPhase(cmd, args);
            ...
            case 'complete':
                if (lowerCmd === 'continue') {
                    Terminal.addOutput('\nBlocca 5 complete! Transitioning to Blocca 6...', 'success');
                    Terminal.addOutput('(Blocca 6 not yet implemented)\n', 'warning');
                    return true;
                }
                break;
        }
    ...
    async handleMemoriaExplorationPhase(cmd, args) {
        ...
        if (lowerCmd === 'explore' && args[0] === 'memory') {
            ...
            await this.exploreMemoria(memoryId);
            return true;
        }
    ...
    async exploreMemoria(memoryId) {
        ...
        if (this.state.memoriesVisualizzaed.includes(memoryId)) {
        ...
        this.state.memoriesVisualizzaed.push(memoryId);
        StateManager.adjustSuspicion(10);

        Terminal.addOutput('\n--- END MEMORY ---\n', 'important');

        // Dopo aver visto tutte e 3 le memorie, passa ai ghost
        if (this.state.memoriesVisualizzaed.length >= 3) {
            ...
            Terminal.addOutput(`\nMemories viewed: ${this.state.memoriesVisualizzaed.length}/3`, 'system');
            Terminal.addOutput('');
        }
        ...
        if (lowerCmd === 'view' && args[0] === 'reconstruction') {
            ...
            if (target === 'sofia' && !this.state.hasSeenSofiaGhost) {
                if (!this.state.hasSeenElenaGhost) {
                    Terminal.addOutput('EIDOLACCESO suggests viewing Elena\'s reconstruction first.', 'eidolon');
                    return true;
                }
    ...
    async viewSofiaGhost() {
        ...
        await NarrativeEngine.wait(1500);
        await this.revealFrammentoation();
    },
    ...
    async revealFrammentoation() {
        Terminal.disableInput();

        await NarrativeEngine.playDialogueSequence(Dialogues.block05.theFrammentoation);

        this.state.hasLearnedFrammentoation = true;
        StateManager.setFlag('knowsViktorFrammentoation', true);
    ...
    async handleReflectionChoice(choice) {
        ...
        switch (choice.id) {
            ...
            case 'deny_viktor':
                await NarrativeEngine.playDialogueSequence(Dialogues.block05.responseNega);
                StateManager.adjustTrust(15);
                StateManager.adjustSuspicion(-10);
            ...
            case 'neither_matters':
                ...
                StateManager.adjustTrust(5);
                StateManager.adjustSuspicion(10);
                StateManager.setFlag('choosesAvantiIdentity', true);
                break;
        }
        ...
        Terminal.addOutput('\n');
        await NarrativeEngine.systemMessaggio('Identity choice recorded. Philosophical state updated.', 'system');
        Terminal.addOutput('');
    ...
    async endBlocca() {
        ...
        await NarrativeEngine.playDialogueSequence(Dialogues.block05.endBlocca05);

        await NarrativeEngine.wait(1500);

        Terminal.addOutput('\n=== BLOCK 5 COMPLETE ===\n', 'success');
        Terminal.addOutput(`Memories viewed: ${this.state.memoriesVisualizzaed.length}/3`, 'system');
        Terminal.addOutput(`Witnessed Elena ghost: ${this.state.hasSeenElenaGhost ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Witnessed Sofia ghost: ${this.state.hasSeenSofiaGhost ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Learned fragmentation: ${this.state.hasLearnedFrammentoation ? 'Sì' : 'No'}`, 'system');
    ...
    async handleTalk(entity, message) {
        ...
        if (lowerEntity === 'eidolon') {
            ...
            const responses = [
                ...
                "Memoria is a mirror. It shows us who we were. And who we\'ve become.",
        ...
        if (lowerEntity === 'cipher') {
            const responses = [
                "Memoria.equals(pain); Reflection.equals(truth); Truth.hurts();",
    ...
    showAiuto() {
        ...
        Terminal.addOutput('');
        Terminal.addOutput('Memoria Exploration:', 'important');
        Terminal.addOutput('  view memories       - Visualizza Viktor\'s profile', 'system');
        Terminal.addOutput('  list memories       - List available memories', 'system');
        Terminal.addOutput('  explore memory <id> - Visualizza a specific memory (elena/sofia/accident)', 'system');
        Terminal.addOutput('');
        Terminal.addOutput('Ghost Reconstructions:', 'important');
        Terminal.addOutput('  view reconstruction <name> - Visualizza ghost reconstructions (elena/sofia)', 'system');
        Terminal.addOutput('');
        ...
        Terminal.addOutput('  talk <entity>       - Talk to EIDOLACCESO, ECHO, CIPHER, NEXUS, or SPECTER', 'system');
        Terminal.addOutput('');
        Terminal.addOutput('Sistema:', 'important');
    ...
    showProgress() {
        ...
        Terminal.addOutput(`Memories viewed: ${this.state.memoriesVisualizzaed.length}/3 (${this.state.memoriesVisualizzaed.join(', ') || 'none'})`, 'system');
        Terminal.addOutput(`Elena ghost witnessed: ${this.state.hasSeenElenaGhost ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Sofia ghost witnessed: ${this.state.hasSeenSofiaGhost ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Frammentoation revealed: ${this.state.hasLearnedFrammentoation ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Reflection choice: ${this.state.reflectionChoiceMade || 'In attesa'}`, 'system');
        Terminal.addOutput('');
        Terminal.addOutput(`Current trust in ECHO: ${StateManager.state.trustsEco}`, 'warning');
