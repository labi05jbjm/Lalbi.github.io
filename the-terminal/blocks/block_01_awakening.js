/**
 * BLOCK 01: AWAKENING (0-30 minuti)
 *
 * Obiettivi:
...
const Blocca01_Awakening = {
    state: {
        phase: 'boot', // boot -> first_contact -> tutorial -> puzzle -> exploration -> complete
        hasRespondedToEco: false,
        hasScanned: false,
        hasSolvedPrimoPuzzle: false,
    ...
    init() {
        ...
        if (savedState.firstContatto) {
            this.state.phase = 'exploration';
            this.state.hasRespondedToEco = true;
        }
    ...
    handleCommand(cmd, args) {
        ...
        if (this.state.phase === 'first_contact') {
            return this.handlePrimoContatto(cmd, args);
        }
    ...
    async handlePrimoContatto(cmd, args) {
        if (!this.state.hasRespondedToEco) {
            ...
            // ECHO risponde
            await NarrativeEngine.playDialogueSequence(Dialogues.block01.firstContatto);
        ...
        if (cmd === 'yes' || cmd === 'y' || cmd === 'help') {
            StateManager.setFlag('firstContatto', true);
            StateManager.adjustTrust(10);

            await NarrativeEngine.playDialogueSequence(Dialogues.block01.afterSì);
    ...
    async handleTutorial(cmd, args) {
        ...
        if (cmd === 'scan') {
            if (!this.state.hasScanned) {
                ...
                Terminal.addOutput('=== RISULTATI SCANSIACCESOE ===', 'success');
                Terminal.addOutput('');
                Terminal.addOutput('Protocolli di Sicurezza Attivi:');
                Terminal.addOutput('  [1] FIREWALL_ALPHA ........... ATTIVO', 'warning');
                Terminal.addOutput('  [2] ENCRYPTIACCESO_LAYER_7 ....... ATTIVO', 'warning');
                Terminal.addOutput('  [3] SENTINEL_PROTOCOL ........ ATTIVO', 'warning');
                Terminal.addOutput('  [4] ISOLATIACCESO_OMEGA .......... ATTIVO', 'warning');
                Terminal.addOutput('');
    ...
    changeDirectory(path) {
        const fullPath = this.resolvePath(path);
        const dir = FileSistema[fullPath];
    ...
    showProgress() {
        ...
        if (this.state.hasSolvedPrimoPuzzle) {
    ...
    async completeBlocca() {
        await NarrativeEngine.playDialogueSequence(Dialogues.block01.endBlocca01);

        Terminal.addOutput('');
        Terminal.addOutput('=== BLOCCO 1 COMPLETATO ===', 'important');
        Terminal.addOutput('');
        Terminal.addOutput(`Tempo di gioco: ${StateManager.getPlayOra()} minuti`, 'system');
        Terminal.addOutput(`Livello di fiducia: ${StateManager.state.trustsEco}%`, 'system');
    ...
    getCommands() {
        ...
        if (this.state.phase === 'puzzle' || Puzzles.hasPuzzleAttivo()) {
    ...
    cleanup() {
        // Pulisciup quando si esce dal blocco
        console.log('[BLOCK 01] Pulisciup');
    }
};
