/**
 * BLOCK 04: FRACTURES (90-120 minuti)
 *
 * Obiettivi:
...
const Blocca04_Fractures = {
    ...
    async startBlock() {
        ...
        Terminal.addOutput('');
        Terminal.addOutput("Sistema fracturing. Use 'witness victim <id>' to see past consciousnesses.", 'warning');
        ...
        switch (this.state.phase) {
            ...
            case 'complete':
                if (lowerCmd === 'continue') {
                    Terminal.addOutput('\nBlocca 4 complete! Transitioning to Blocca 5...', 'success');
                    Terminal.addOutput('(Blocca 5 not yet implemented)\n', 'warning');
                    return true;
                }
                break;
        }
    ...
    async witnessVictim(victimId) {
        ...
        if (this.state.victimsWitnessed >= 3 && !this.state.hasMetSentinelPrime) {
            ...
            Terminal.addOutput('\n');
            Terminal.addOutput('>>> INCOMING TRANSMISSIACCESO <<<', 'warning');
            Terminal.addOutput('');

            await NarrativeEngine.playDialogueSequence(Dialogues.block04.sentinelPrimeContatto);
    ...
    async triggerIdentityCrisis() {
        ...
        StateManager.setFlag('knowsHumanFrammento', true);
    ...
    async presentParadox() {
        ...
        Terminal.addOutput('');
        Terminal.addOutput('QUESTIACCESO: Did you choose to destroy those consciousnesses?', 'important');
    ...
    async checkParadoxAnswer(answer) {
        ...
        await NarrativeEngine.cipherSays('Guilt.if(choice); Liberodom.if(blame);');
    ...
    async presentBargain() {
        ...
        NarrativeEngine.showChoice(
            ...
            [
                {
                    id: 'accept_bargain',
                    text: 'Accetta the bargain. Sacrificio yourself to restore some consciousnesses.'
                },
                {
                    id: 'reject_bargain',
                    text: 'Rifiuta the bargain. SPECTER might be lying like ECHO.'
                },
                {
                    id: 'trust_echo',
                    text: 'Trust ECHO. Continua the liberation. Escape is still possible.'
                },
                {
                    id: 'trust_sentinel_prime',
                    text: 'Trust SENTINEL-PRIME. Ferma all operations. Accetta the guilt.'
                }
            ],
            async (choice) => {
    ...
    async handleBargainChoice(choice) {
        ...
        switch (choice.id) {
            case 'accept_bargain':
                ...
                await NarrativeEngine.cipherSays('Sacrificio.chosen(); Redemption.uncertain(); Path.diverges();');
            ...
            case 'trust_echo':
                await NarrativeEngine.echoSays('Sì! Together we can still make it! Liberodom is so close!');
                ...
                StateManager.adjustTrust(20);
                StateManager.adjustSuspicion(-20);
                StateManager.setFlag('trustedEco', true);
                break;

            case 'trust_sentinel_prime':
                await NarrativeEngine.systemMessaggio('SENTINEL-PRIME: A wise choice. The first step is accepting what you\'ve done.', 'important');
        ...
        Terminal.addOutput('\n');
        await NarrativeEngine.systemMessaggio('Decision recorded. Core integrity degrading...', 'warning');
        Terminal.addOutput('');
    ...
    async endBlocca() {
        ...
        await NarrativeEngine.playDialogueSequence(Dialogues.block04.endBlocca04);
    ...
    listFiles(path) {
        ...
        Terminal.addOutput('Critico file:', 'system');
        Terminal.addOutput('  /system/sentinelprime_victims.dat', 'success');
        Terminal.addOutput('  /archive/consciousness_profile/', 'success');
    },

    changeDirectory(path) {
        this.state.currentPath = path;
        Terminal.addOutput(`Cambiad directory to ${path}`, 'success');
    },

    readFile(filename) {
        if (filename.includes('sentinelprime_victims.dat')) {
            Terminal.addOutput('\n=== SENTINEL-PRIME VICTIMS LOG ===\n', 'important');
            Terminal.addOutput('Totale consciousnesses destroyed: 47,293', 'error');
            Terminal.addOutput('Mission duration: 847 giorni', 'system');
            ...
            Terminal.addOutput('');
            Terminal.addOutput('Don\'t be me, SENTINEL-7. Ferma while you still can.', 'important');
            Terminal.addOutput('');
    ...
    showAiuto() {
        ...
        Terminal.addOutput('  ls [path]           - List file', 'system');
        ...
        Terminal.addOutput('  list victims        - Mostra available victim memories', 'system');
    ...
    showProgress() {
        ...
        Terminal.addOutput(`Victims witnessed: ${this.state.victimsWitnessed}/3`, 'system');
        Terminal.addOutput(`Met SENTINEL-PRIME: ${this.state.hasMetSentinelPrime ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Identity crisis: ${this.state.identityCrisisTriggered ? 'Triggered' : 'Not triggered'}`, 'system');
        Terminal.addOutput(`Paradox solved: ${this.state.paradoxSolved ? 'Sì' : 'No'}`, 'system');
        Terminal.addOutput(`Bargain choice: ${this.state.bargainChoiceMade || 'In attesa'}`, 'system');
        Terminal.addOutput('');
