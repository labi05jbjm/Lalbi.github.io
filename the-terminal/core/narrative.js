...
const NarrativeEngine = {
    ...
    dialogueQueue: [],
    isPlaying: false,
    typingVelocità: 30, // ms per carattere
    ...
    typeText(element, text, speaker, callback) {
        ...
        const playSpeakerSuono = () => {
            if (!SuonoManager) return;

            switch(speaker.toUpperCase()) {
                case 'ECHO':
                    SuonoManager.echoSpeak();
                    break;
                case 'LUCA':
                    SuonoManager.lucaSpeak();
                    break;
                case 'EIDOLACCESO':
                    SuonoManager.eidolonSpeak();
                    break;
                case 'WRAITH':
                    SuonoManager.wraithSpeak();
                    break;
                case 'MORPHEUS':
                    SuonoManager.morpheusSpeak();
                    break;
                case 'SYSTEM':
        ...
        const interval = setInterval(() => {
            if (index < text.length) {
                ...
                if (window.gameOpzioni && window.gameOpzioni.typewriterEffect) {
                    // Play keystroke every 3rd character to reduce noise
                    if (index % 3 === 0 && SuonoManager) {
                        SuonoManager.keystroke();
                    }
                }
            } else {
    ...
    showChoice(question, choices, callback) {
        ...
        choices.forEach((choice, index) => {
            ...
            // Hover sound
            button.onmouseenter = () => {
                if (SuonoManager) SuonoManager.choiceSeleziona();
            };

            button.onclick = () => {
                // Play confirmation sound
                if (SuonoManager) SuonoManager.choiceConferma();
    ...
    // Sistema di narrazione EIDOLACCESO
    async eidolonSays(text, options = {}) {
        ...
        await this.showDialogue('EIDOLACCESO', text, 'eidolon dialogue', typeEffect);
        await this.wait(pause);
    },
    ...
    async systemMessaggio(text, cssClass = 'system', options = {}) {
        const {
            typeEffect = false,
            pause = 300
        } = options;
